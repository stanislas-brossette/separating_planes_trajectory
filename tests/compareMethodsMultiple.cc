...
struct Summary
{
  double averageIter;
  double totalTime;
  double averageTimePerIter;
  double successRate;
  void print() const
  {
    std::cout << "averageIter: " << averageIter << std::endl;
    std::cout << "totalTime: " << totalTime << std::endl;
    std::cout << "averageTimePerIter: " << averageTimePerIter << std::endl;
    std::cout << "successRate: " << successRate << std::endl;
  }
};

Summary fillSummary(const std::vector<logResult> res)
{
  double timePerIter;
  Summary summary;
  for (auto s : res)
  {
    ...
    {
      summary.successRate ++;
      summary.averageIter += s.iteration_;
      timePerIter = s.time_/s.iteration_;
      summary.totalTime += s.time_;
      summary.averageTimePerIter += timePerIter;
    }
  }
  summary.successRate = 100*summary.successRate / res.size();
  summary.averageIter = summary.averageIter/res.size();
  summary.totalTime = summary.totalTime/res.size();
  summary.averageTimePerIter = summary.averageTimePerIter/res.size();
  summary.averageTimePerIter = summary.averageTimePerIter/CLOCKS_PER_SEC;
  return summary;
}
...
int main(void)
{
  ...
  std::string ymlPath = testDir + "/compareMethodsMultiple.yml";
  ...
  Eigen::VectorXd allRegVal = config["allRegVal"].asVectorXd();
  long nRegVal = allRegVal.size();
  std::cout << "allRegVal: " << allRegVal << std::endl;
  std::map<double, std::vector<Summary>> allSummariesManifolds, allSummariesRealSpace;
  int minCubes = config["minCubes"].asInt();
  int maxCubes = config["maxCubes"].asInt();
  int nExp = maxCubes - minCubes;

  for (long i = 0; i < nRegVal; i++) 
  {
    allSummariesManifolds[allRegVal[i]].resize(static_cast<size_t>(nExp));
    allSummariesRealSpace[allRegVal[i]].resize(static_cast<size_t>(nExp));
  }

  for (int nCubes = minCubes; nCubes <= maxCubes; nCubes++)
  {
    ...
    for (int indexReg = 0; indexReg < allRegVal.size(); indexReg++) 
    {
      for (int i = 0; i < config["numberOfTests"].asInt(); i++) 
      {
        std::cerr << "i: " << i << std::endl;
        ...
        resManifold.push_back(solveOnManifold(*Mptr, inits[static_cast<size_t>(i)], ymlPath, manifoldPbName));
        //resSO3noS2.push_back(solveOnSO3noS2(*MSO3noS2ptr, v0, ymlPath, SO3noS2PbName));
        resRealSpace.push_back(solveOnR(*Rptr, inits[static_cast<size_t>(i)], ymlPath, RealSpacePbName));
      }
      ...
      if(config["timeLog"])
      {
        ...
        allSummariesManifolds.at(allRegVal[indexReg])
            .push_back(fillSummary(resManifold));
        allSummariesRealSpace.at(allRegVal[indexReg])
            .push_back(fillSummary(resRealSpace));
