...
int main(void)
{
  ...
  std::string ymlPath = testDir + "/stackPointCloud.yml";
  ProblemConfig config(ymlPath);
  int nPC = config["nPointClouds"];

  mnf::CartesianProduct* M = PointCloudStackProblemOnManifold::buildManifold(nPC);

  M->display();

  PointCloudStackProblemOnManifold myProb(*M, ymlPath);
